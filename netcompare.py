...
def clean_list(list, vendor, config):
    list_clean = []
...
def cmp(origin, target):
    args = cli_parser("")

    args.origin = origin
    args.target = target
    args.vendor = "ios"
    args.config = "netcompare.yml"
...
def compareConfigs(origin_list, target_list):
    ...
    origin_list = clean_list(origin_list, args.vendor, config)
    target_list = clean_list(target_list, args.vendor, config)
